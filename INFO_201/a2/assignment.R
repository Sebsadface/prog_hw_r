...
has_more_zs <- function(string1, string2) {
  if(sum(str_count(string1, pattern = c("z", "Z"))) > 
     sum(str_count(string2, pattern = c("z", "Z")))) 
  {
    return(string1)
  }
...
more_zs <- has_more_zs("Zach zaps Zoe to make her eat zink pill.", 
                       "Zebras are walking down a zigzaging hill by a zoo.")
...
movies <- c("La La Land", "Inception", "Interstellar", "Star Wars", 
            "Green Book", "The Godfather")
